...
#@jit(nopython=True)
def define_transition_matrix(B, tol=0, score=[1,0.01]):
    ...
    Returns:
        T: Transition matrix
    """
    col = np.ones((B,)) * score[1]
    col[0:tol+1] = np.ones((tol+1,)) * score[0]
...
#@jit(nopython=True)
def compute_trajectory_CR(Z, T_coef, F_coef_hertz, constraint_region=None, tol=5, score=[1,0.01]):
    ...
    if constraint_region is not None:
        ...
        for row_idx in range(constraint_region.shape[0]):
            ...
            T = define_transition_matrix(cur_Z.shape[0], tol=tol, score=score)
...
def compute_traj_from_audio(x, Fs=22050, N=1024, H=128, R=10, F_min=55, F_max=1760, num_harm=10,
            freq_smooth_len=11, alpha=0.9, gamma=0, constraint_region=None, tol=5, score=[1,0.01]):
    ...
    index_CR = compute_trajectory_CR(Z, T_coef, F_coef_hertz, constraint_region, tol=tol, score=score)
